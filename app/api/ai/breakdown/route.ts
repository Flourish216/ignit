...
export async function POST(request: Request) {
  ...
  try {
    ...
    const userPrompt = isChinese
      ? `请帮我整理这个 Spark："${idea}"。\n\nSpark 指的是用户想找一个人一起开始的事情，比如做东西、学习、运动、出门、创作。\n\n请用 JSON 格式返回一张清楚、自然、不像约会软件也不像职场平台的 Spark Card。JSON 字段名必须保持英文，字段内容请使用简体中文。`
      : `Please structure this Spark: "${idea}". A Spark is something the user wants to start with someone else, such as building, learning, moving, going somewhere, or creating. Keep the JSON field names exactly as requested and write all content values in English.`

    const systemPrompt = `You are a Spark structuring assistant for Ignit, a product that helps people find someone to start something meaningful with.

Ignit is not a dating app, not LinkedIn, and not only for software projects.

Your goal is to turn a rough user Spark into a simple Spark Card that another person can quickly understand and respond to.
...
{
  "title": "Short Spark title, max 60 characters",
  "category": "One of: Build, Learn, Move, Go, Create",
